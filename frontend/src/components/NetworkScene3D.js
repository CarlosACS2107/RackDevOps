...
export default function NetworkScene3D({ config }) {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < Math.min(config.servers, 4); i++) {
            ...
            rackGroup.add(srv);
            // led del servidor
            const led = new THREE.Mesh(new THREE.SphereGeometry(0.04), new THREE.MeshStandardMaterial({ color: 0x00ff88, emissive: 0x00ff88, emissiveIntensity: 2 }));
        ...
        function placeDevices(count, color, radius, angleStart, angleEnd, y = 0) {
            ...
            for (let i = 0; i < total; i++) {
                ...
                mesh.position.set(Math.cos(rad) * radius, y, Math.sin(rad) * radius);
                scene.add(mesh);
                group.push(mesh);
                // línea al rack
                const points = [
                ...
                const line = new THREE.Line(new THREE.BufferGeometry().setFromPoints(points), new THREE.LineBasicMaterial({ color, opacity: 0.3, transparent: true }));
                scene.add(line);
            }
            return group;
        }
    ...
    return (_jsx("div", { ref: mountRef, style: { width: '100%', height: '100%', cursor: 'grab' } }));
}
